...
class _CountdownScreenState extends State<CountdownScreen> {
  ...
  Future<void> _initializeTimerState() async {
    ...
    if (endTime != null &&
        runningTask == widget.task.id &&
        !wasManuallyStopped) {
      final now = DateTime.now().millisecondsSinceEpoch;
      final remaining = (endTime - now) > 0 ? (endTime - now) ~/ 1000 : 0;

      if (remaining > 0) {
        _startForegroundCountdown(remaining);
      } else {
  ...
  void _startForegroundCountdown(int seconds) {
  ...
  void _onTimerFinish() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'Focus session for "${widget.task.title}" is complete! You can now mark it as done.'),
  ...
  Future<void> _loadQuotes() async {
    try {
      ...
      final quoteFiles = manifestMap.keys
          .where((String key) => key.startsWith('assets/quotes/'))
          .toList();
      ...
      if (quoteFiles.isNotEmpty) {
        ...
        final quotesString =
            await DefaultAssetBundle.of(context).loadString(randomFile);
        quotesList = LineSplitter.split(quotesString)
        ...
        child: Column(
          ...
          children: [
            Text(_formatDuration(_remainingSeconds),
                style: theme.textTheme.displayLarge?.copyWith(fontSize: 80)),
            const SizedBox(height: 20),
            if (widget.library != null)
              Chip(
                avatar: const Icon(Icons.library_books_outlined, size: 18),
                label: Text(widget.library!.name,
